...
function backup_step1(){
 ...
 $output .= "<center>
		<br /><fieldset class=\"tquarter_frame\">
		<legend>{$lang_backup['backup_options']}</legend>
		  <table class=\"hidden\">
          <tr><td colspan=\"2\">{$lang_backup['select_option']}:</td></tr>";
 $output .= "<tr><td><form action=\"backup.php\" method=\"get\" name=\"form\">
 ...
 $output .= "<tr><td colspan=\"2\" align=\"left\"><input type=\"checkbox\" name=\"struc_backup\" value=\"1\" /> {$lang_backup['save_table_struc_backup']}.</td></tr>
		<tr><td colspan=\"2\" align=\"left\"><input type=\"checkbox\" name=\"save_all_realms\" value=\"1\" checked=\"checked\" /> {$lang_backup['save_all_realms']}.</td></tr>
		</form></td></tr>
...
case "load":
	$output .= "<center><fieldset class=\"tquarter_frame\">
				<legend>{$lang_backup['select_file']}</legend>
				<br /><table class=\"hidden\">";

	switch ($backup_from_to) {
		case "file":
			$output .= "<tr><td colspan=\"2\">{$lang_backup['max_file_size']} : $upload_max_filesize bytes (".round ($upload_max_filesize/1024/1024)." Mbytes)<br />
			{$lang_backup['use_ftp_for_large_files']}.</td></tr><tr>";
			$output .= "<td><form enctype=\"multipart/form-data\" action=\"backup.php?action=dobackup&amp;backup_action=$backup_action&amp;backup_from_to=$backup_from_to\" method=\"post\" name=\"form\">
			...
			$output .= "<option value=\"{$realm_db['name']}\">{$realm_db['name']}</option>
					...
					makebutton($lang_backup['upload'], "javascript:do_submit()",80);
		break;

		case "web":
			$output .= "<tr><td><form action=\"backup.php?action=dobackup&amp;backup_action=$backup_action&amp;backup_from_to=$backup_from_to\" method=\"post\" name=\"form\">
						<select name=\"use_db\">";
			foreach ($characters_db as $db) $output .= "<option value=\"{$db['name']}\">{$db['name']}</option>";
			$output .= "<option value=\"{$realm_db['name']}\">{$realm_db['name']}</option>
						</select>			
			...
			$output .= "</select></form></td><td>";
			makebutton($lang_backup['go'], "javascript:do_submit()",80);
		break;
		
		case "acc_on_file":
			$output .= "<tr><td colspan=\"2\">{$lang_backup['enter_acc_name']}:</td></tr>
						<tr><td>
...
function dobackup(){
 ...
 if (("load" == $backup_action)&&("file" == $backup_from_to)){
	if (!eregi("(\.(sql|qbquery))$",$_FILES["uploaded_file"]["name"])) error($lang_backup['upload_sql_file_only']);
	...
	if (file_exists("$backup_dir/$file_name_new")){
	...
	} else error($lang_backup['file_not_found']);
		...
		if (file_exists("$backup_dir/$file_name")){
			...
			if ($use_db == $realm_db['name']) $queries = run_sql_script($realm_db['addr'], $realm_db['user'], $realm_db['pass'], $realm_db['name'], "$backup_dir/$file_name",false);
				...
				foreach ($characters_db as $db)
					if ($use_db == $db['name']) $queries = run_sql_script($db['addr'], $db['user'], $db['pass'], $db['name'], "$backup_dir/$file_name",false);
				}
			...
			if($save_all_realms) $temp_id = "all_realms";
				else $temp_id = "realmid_".$realm_id;
			...
			redirect("backup.php?error=2");
   			exit();
 } else if (("save" == $backup_action)&&("acc_on_file" == $backup_from_to)){
		...
		if (file_exists($file_tmp)){
			...
			if ($use_db == $realm_db['name']) $queries = run_sql_script($realm_db['addr'], $realm_db['user'], $realm_db['pass'],$realm_db['name'], "$backup_dir/accounts/$file_dir/$file_name.sql",true);
				...
				foreach ($characters_db as $db)
					if ($use_db == $db['name']) $queries = run_sql_script($db['addr'], $db['user'], $db['pass'],$db['name'], "$backup_dir/accounts/$file_dir/$file_name.sql",true);
				}
...
switch ($err) {
case 1:
   $output .= "<h1><font class=\"error\">{$lang_global['empty_fields']}</font></h1></div>";
   break;
case 2:
   $output .= "<h1><font class=\"error\">{$lang_backup['backup_finished']}</font></h1></div>";
   break;
case 3:
	$output .= "<h1>{$lang_backup['select_backup']}</h1></div>";
   break;
case 4:
	if(isset($_GET['tot'])) $total_queries = $_GET['tot'];
		else $total_queries = NULL;
	$output .= "<h1><font class=\"error\">{$lang_backup['file_loaded']} $total_queries {$lang_backup['que_executed']}.</font></h1></div>";
	break;
default: //no error
	$output .= "<h1>{$lang_backup['backup_acc']}</h1></div>";
	$output .= "<center><font class=\"large\">{$lang_backup['tables_to_save']}:</font><br />
			<table width=\"700\" class=\"hidden\">
				<tr><td>";
	foreach ($tables_backup_realmd as $value) $output .= " {$realm_db['name']}.$value / ";
	foreach ($tables_backup_characters as $value) $output .= " $value / ";
